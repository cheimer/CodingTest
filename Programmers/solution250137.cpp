...
int solution(vector<int> bandage, int health, vector<vector<int>> attacks) {
    ...
    for(int time = 1; time <= attacks[attacks.size() - 1][ATTACK_TIME]; time++)
    {
        if(time == attacks[attackNum][ATTACK_TIME])
        {
            currentHealth -= attacks[attackNum][ATTACK_DAMAGE];
            if(currentHealth <= 0)
            {
                break;
            }
...
// bandage 시간, 초당 회복량, 추가 회복량
// attacks 공격 시간, 피해량 && 공격 시간 기준 오름차순, 공격시간은 모두 다름

// answer = 남은 체력 || if < 0 return -1